...
type HTTPClient interface {
	SendReceive(url, method string, in interface{}) (out interface{}, err error)
}

type EchoHTTPClient struct {
}

func (c *EchoHTTPClient) SendReceive(url, method string, in interface{}) (out interface{}, err error) {
...
type HTTPService struct { // 也是一个HTTPClient
	log    Logger
	client HTTPClient
	cache  Cache
	// :其他字段不使用依赖
...
func NewService(client HTTPClient, log Logger, cache Cache) (s *HTTPService) {
	s = &HTTPService{}
	s.log = log
	s.client = client
	s.cache = cache
...
func (s *HTTPService) SendReceive(url, method string, in interface{}) (out interface{}, err error) {
...
func main() {
	log := StdoutLogger{}      // 具体类型
	client := EchoHTTPClient{} // 具体类型
	cache := MapCache{}        // 具体类型
	// 创建一个注入所有依赖的服务
